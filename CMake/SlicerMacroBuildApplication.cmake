...
macro(slicerMacroBuildApplication)
  ...
  if(WIN32)
    if(Slicer_USE_PYTHONQT)
	  ...
	  #PYTHON_LIBRARY: optimized;C:/Library/Python27/libs/python27.lib;debug;C:/Library/Python27/libs/python27_d.lib
	  # we must create array to support debug and release copy mechanism
	  
	  set(SLICER_PYTHONLIB_DEBUG)
	  set(SLICER_PYTHONLIB_RELEASE)
	  
	  list(LENGTH PYTHON_LIBRARY len1)
	  
	  math(EXPR len2 "${len1} - 1")
	  
		foreach(val RANGE ${len2})
		  list(GET PYTHON_LIBRARY ${val} val1)

		  message(STATUS "${val} = ${val1}")
		  
		  math(EXPR idNext "${val} + 1")
		  
		  if(val1 STREQUAL optimized)
				list(GET PYTHON_LIBRARY ${idNext} valLib)
				set(SLICER_PYTHONLIB_RELEASE ${valLib})
		  endif()
		  
		  if(${val1} STREQUAL debug)
				list(GET PYTHON_LIBRARY ${idNext} valLib)
				set(SLICER_PYTHONLIB_DEBUG ${valLib})
		  endif()		  
		  
		  
		endforeach()	 

	message(STATUS "SLICER_PYTHONLIB_DEBUG: ${SLICER_PYTHONLIB_DEBUG} , SLICER_PYTHONLIB_RELEASE: ${SLICER_PYTHONLIB_RELEASE}")
